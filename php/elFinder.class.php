...
class elFinder {
    ...
    public function __construct($options=array()) {
        ...
        if ($this->_options['connectorController']){
            $this->connectorController = $this->_options['connectorController'];
        }
    }
    ...
    protected function _upload()
    {
            ...
            if (!empty($_FILES['upload']['name'][$i])) {
                ...
                if ($_FILES['upload']['error'][$i] > 0) {
                    ...
                    if (!@move_uploaded_file($_FILES['upload']['tmp_name'][$i], $file)) {
                        ...
                        $thumbnailName = $this->_hash($file);
                        $filesUploaded[] = array(
                            'fileName' => $file,
                            'thumbnailName' => $thumbnailName.'.'.$path_parts['extension']
                        );
                        $this->_result['select'][] = $thumbnailName;
                    }
                }
            }
        }
        ...
        if ($errCnt == $total) {
            ...
            if(is_object($this->connectorController)){
                $this->connectorController->uploaded($filesUploaded);
            }
            $this->_content($dir);
        }
